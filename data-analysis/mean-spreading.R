...
qqnorm(data_mSpreading$meanSpreading, main="QQPlot of M-Spreading distribution", 
       ylab="M-Spreading Sample Quantiles")
...
for(proj in unique(data_mSpreading$project)){
  dataTestmSpreading <- data_mSpreading %>%
    filter(project==proj) %>%
    mutate(meanSpreading = meanSpreading) %>%
  ...
  for(proj in unique(data_mSpreading$project)){
    for(alg in unique(data_mSpreading$algorithm)){
      result <- data_mSpreading %>%
        ...
        resample::bootstrap(mean(meanSpreading), R = 10000) %>%
